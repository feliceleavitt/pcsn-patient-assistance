...
export async function PATCH(
  ...
  if (request.headers.get("content-type")?.includes("multipart/form-data")) {
    ...
    for (const file of formData.getAll("documents")) {
      ...
      await supabase.from("documents").insert({ submission_id: submissionId, original_filename: file.name, document_type: "volunteer_upload", storage_path: storagePath, mime_type: file.type || "application/octet-stream", byte_size: file.size, encryption_iv: encrypted.iv, encryption_tag: encrypted.tag });
    }
